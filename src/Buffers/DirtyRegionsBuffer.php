...
use Microscrap\GFX\PhpdaFruit\Factory\Buffers\DirtyRegionsBufferFactory;
...
/**
 * A retained canvas that ships only the rectangles that changed.
 *
 * Where {@see PageSegmentBuffer} slices updates into fixed 8-row pages for
 * page-addressed mono panels, this buffer coalesces writes into arbitrary
 * dirty rectangles and emits one PARTIAL DumpedBuffer per region. That maps
 * onto row-major TFTs (ST77xx/ILI9xxx) whose CASET/RASET address window can be
 * set to any rectangle, so a small change ships only its bounding box instead
 * of re-blasting the whole frame.
 *
 * Overlapping or touching rectangles are merged as they are marked, keeping the
 * region list small and free of double-sent pixels.
 */
